...
- **SubAgent 架構**：後續問題採用三階段 SubAgent 設計 —（1）結合對話歷史將後續問題解析為獨立問題，（2）在不帶歷史的情況下執行主動式工具呼叫迴圈以節省 Token，（3）結合歷史上下文合成最終回答。此架構降低了 Token 消耗，並支援每個工作階段最多 **25 組問答**
...
**為何重要：** 在先前的設計中，對話歷史會隨著工具迴圈中的每次 LLM 呼叫一同傳送（5–50 次）。使用 SubAgent 後，歷史僅傳送兩次（階段 1 + 3），使得 Token 成本幾乎與歷史長度無關。因此可支援每個工作階段 **25 組問答**（50 turns）而不會溢出上下文。

頂部欄分別顯示 **Main**（階段 1 + 3）與 **Sub**（階段 2）的 Token 用量。點擊 **Main** 會開啟彈窗顯示 LLM 記憶中的確切對話內容，並提供按鈕可將歷史記錄下載為 Markdown。
